...
public final class InternalProportionalSumHistogram extends InternalMultiBucketAggregation<InternalProportionalSumHistogram, InternalProportionalSumHistogram.Bucket>
    ...
    public static class Bucket extends InternalMultiBucketAggregation.InternalBucket implements Histogram.Bucket, HistogramBucketWithValue, KeyComparable<Bucket> {
        ...
        public Object getKey() {
            if (format instanceof DocValueFormat.DateTime) {
        ...
        Bucket reduce(List<Bucket> buckets, AggregationReduceContext context) {
            ...
            InternalAggregations reducedAggs = InternalAggregations.reduce(aggregationsList, context);
            return new Bucket(
                    key, // Using the existing bucket's key
                    docCount,
                    value,
                    keyed,
                    format,
    ...
    private List<Bucket> reduceBuckets(List<InternalProportionalSumHistogram> aggregations, AggregationReduceContext reduceContext) {
        ...
        for (InternalProportionalSumHistogram aggregation : aggregations) {
            if (!aggregation.buckets.isEmpty()) {
                pq.add(new IteratorAndCurrent(aggregation.buckets.iterator()));
            }
        }
    ...
    private void addEmptyBuckets(List<Bucket> list, AggregationReduceContext reduceContext) {
        ...
        while (iter.hasNext()) {
            ...
            if (lastBucket != null) {
                ...
                assert key == nextBucket.key : "key: " + (key +828000000) + ", nextBucket.key: " + nextBucket.key;
            }
    ...
    public InternalAggregation reduce(List<InternalAggregation> aggregations, AggregationReduceContext reduceContext) {
        AggregatorReducer aggregatorReducer = getLeaderReducer(reduceContext, aggregations.size());
        for (InternalAggregation agg : aggregations) {
            aggregatorReducer.accept(agg);
        }
        return aggregatorReducer.get();
    }

    @Override
    protected AggregatorReducer getLeaderReducer(AggregationReduceContext reduceContext, int size) {
        return new AggregatorReducer() {
            final List<InternalProportionalSumHistogram> aggregations = new ArrayList<>(size);

            @Override
            public void accept(InternalAggregation aggregation) {
                aggregations.add((InternalProportionalSumHistogram) aggregation);
            }
            ...
            public InternalAggregation get() {
                ...
                return new InternalProportionalSumHistogram(
                        getName(), reducedBuckets, order, minDocCount, offset,
                        emptyBucketInfo, format, keyed, getMetadata()
                );
            }
        };
    }
