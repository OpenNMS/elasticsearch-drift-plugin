...
public class ProportionalSumAggregationBuilder extends MultiValuesSourceAggregationBuilder<ProportionalSumAggregationBuilder> {
    ...
    private static final DateMathParser EPOCH_MILLIS_PARSER =
            DateFormatter.forPattern("epoch_millis").toDateMathParser();
    ...
    private static ParseField ZONE = new ParseField("time_zone");
...
//        dateFieldUnits.put("quarter", Rounding.DateTimeUnit.QUARTER);
//        dateFieldUnits.put("1q", Rounding.DateTimeUnit.QUARTER);
    ...
    static {
        ...
        PARSER.declareString(ProportionalSumAggregationBuilder::timeZone, ZONE);
    }
    ...
    public ProportionalSumAggregationBuilder timeZone(String tz) {
        if (tz == null) {
            throw new IllegalArgumentException("[time_zone] must not be null: [" + name + "]");
        }
        this.timeZone = ZoneId.of(tz);
        return this;
    }

    /** (existing) Set the time zone on this builder, as a ZoneId */
    public ProportionalSumAggregationBuilder timeZone(ZoneId tz) {
        this.timeZone = tz;
        return this;
    }

    /**
     * Get the current time zone, or null if none set.
     */
    ...
    protected MultiValuesSourceAggregatorFactory innerBuild(AggregationContext context, Map<String, ValuesSourceConfig> configs,
        ...
        final ZoneId tz = (timeZone != null ? timeZone : ZoneOffset.UTC);
