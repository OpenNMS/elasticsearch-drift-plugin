...
/**
 * Based on org.elasticsearch.search.aggregations.bucket.histogram.HistogramAggregationBuilder (from ES 7.11.0).
 *
 * The only change compared to the original class is that a {@link FlowHistogramAggregator} is used as the
 * aggregation supplier in the {@link #innerBuild(QueryShardContext, ValuesSourceConfig, AggregatorFactory, AggregatorFactories.Builder)}
 * method.
 */
public class FlowHistogramAggregationBuilder extends ValuesSourceAggregationBuilder<FlowHistogramAggregationBuilder> {

    public static final String NAME = "flow_histogram";
    ...
    static final InstantiatingObjectParser<DoubleBounds, Void> DOUBLE_BOUNDS_PARSER;
    ...
    static {
        ...
        DOUBLE_BOUNDS_PARSER = parser.build();
    }


    public static final ObjectParser<FlowHistogramAggregationBuilder, String> PARSER =
            ObjectParser.fromBuilder(NAME, FlowHistogramAggregationBuilder::new);

    static {
        ValuesSourceAggregationBuilder.declareFields(PARSER, true, true, false);

        PARSER.declareDouble(FlowHistogramAggregationBuilder::interval, Histogram.INTERVAL_FIELD);

        PARSER.declareDouble(FlowHistogramAggregationBuilder::offset, Histogram.OFFSET_FIELD);

        PARSER.declareBoolean(FlowHistogramAggregationBuilder::keyed, Histogram.KEYED_FIELD);

        PARSER.declareLong(FlowHistogramAggregationBuilder::minDocCount, Histogram.MIN_DOC_COUNT_FIELD);

        PARSER.declareField(FlowHistogramAggregationBuilder::extendedBounds, parser -> DOUBLE_BOUNDS_PARSER.apply(parser, null),
            Histogram.EXTENDED_BOUNDS_FIELD, ObjectParser.ValueType.OBJECT);

        PARSER.declareField(FlowHistogramAggregationBuilder::hardBounds, parser -> DOUBLE_BOUNDS_PARSER.apply(parser, null),
            Histogram.HARD_BOUNDS_FIELD, ObjectParser.ValueType.OBJECT);

        PARSER.declareObjectArray(FlowHistogramAggregationBuilder::order, (p, c) -> InternalOrder.Parser.parseOrderParam(p),
    ...
    public FlowHistogramAggregationBuilder(String name) {
        super(name);
    }

    public FlowHistogramAggregationBuilder(FlowHistogramAggregationBuilder clone,
    ...
    protected AggregationBuilder shallowCopy(AggregatorFactories.Builder factoriesBuilder, Map<String, Object> metadata) {
        return new FlowHistogramAggregationBuilder(this, factoriesBuilder, metadata);
    }

    /** Read from a stream, for internal use only. */
    public FlowHistogramAggregationBuilder(StreamInput in) throws IOException {
        super(in);
    ...
    public FlowHistogramAggregationBuilder interval(double interval) {
        if (interval <= 0) {
    ...
    public FlowHistogramAggregationBuilder offset(double offset) {
        this.offset = offset;
        return this;
    }
     ...
     *             are not finite.
     */
    public FlowHistogramAggregationBuilder extendedBounds(double minBound, double maxBound) {
     ...
     *             are not finite.
     */
    public FlowHistogramAggregationBuilder extendedBounds(DoubleBounds extendedBounds) {
        if (extendedBounds == null) {
     ...
     */
    public FlowHistogramAggregationBuilder hardBounds(DoubleBounds hardBounds) {
        if (hardBounds == null) {
    ...
    public FlowHistogramAggregationBuilder order(BucketOrder order) {
        if (order == null) {
     ...
     * ordering.
     */
    public FlowHistogramAggregationBuilder order(List<BucketOrder> orders) {
        if (orders == null) {
    ...
    public FlowHistogramAggregationBuilder keyed(boolean keyed) {
        this.keyed = keyed;
        return this;
    }
    ...
    public FlowHistogramAggregationBuilder minDocCount(long minDocCount) {
        if (minDocCount < 0) {
    ...
    protected ValuesSourceAggregatorFactory innerBuild(QueryShardContext context,
        ...
        return new FlowHistogramAggregatorFactory(name, config, interval, offset, order, keyed, minDocCount, extendedBounds,
    ...
    public boolean equals(Object obj) {
        ...
        FlowHistogramAggregationBuilder other = (FlowHistogramAggregationBuilder) obj;
