...
if [ "$(ls -1 target/artifacts/*.rpm | wc -l)" -eq 0 ]; then
  echo "ERROR: no .rpm files found in assembly/ from a previous build step"
  exit 1
fi
for FILE in target/artifacts/*.rpm; do
...
# shellcheck disable=SC2012
if [ "$(ls -1 target/artifacts/*.deb | wc -l)" -eq 0 ]; then
  echo "ERROR: no .deb files found in assembly/ from a previous build step"
  exit 1
fi
for FILE in target/artifacts/*.deb; do
  # give it 3 tries then die
